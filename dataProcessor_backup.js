(function() {
    ...
    const dataProcessor = {
        ...
        calculateBuildMetrics: (targetBuilds) => {
            ...
            // Use proper BOM hierarchy - each component's parent is determined by indentation structure
            const parentStack = [{ level: 0, requirement: targetBuilds }];

            state.bomData.forEach(item => {
                const currentLevel = item.bomLevel;
                
                // Remove items from stack until we find the immediate parent level
                while (parentStack.length > 1 && parentStack[parentStack.length - 1].level >= currentLevel) {
                    parentStack.pop();
                }
                
                // The current top of stack is the parent
                const parentRequirement = parentStack[parentStack.length - 1].requirement;
                ...
                if (item.componentId === '7000030' || item.componentId === '15000174') {
                    console.log(`FIXED CALC - ${item.componentId}: level=${currentLevel}, parentReq=${parentRequirement}, gross=${grossRequirement}, onHand=${item.onHand}, shortfall=${netQtyToBuild}`);
                }

                // Push this component onto the stack as a potential parent
                parentStack.push({ level: currentLevel, requirement: netQtyToBuild });
